...
def main_d():
  time = 20
  mse_ls = np.zeros(time)
  best_gamma = np.zeros(time)
  best_sigma = np.zeros(time)
  mse_ls_train = np.zeros(time)
  for t in range(time):
    ...
    for i in range(len(gamma)):
      for j in range(len(sigma)):
        ...
        y_predict_ = np.zeros(test_arr.shape[0])#record mse on test
    ...
    #compute mse on train_set
    K_best = K(train_arr[:,0:12],best_sigma[t])
    alpha_best = alpha(K_best,best_gamma[t],train_arr[:,12])
    y_predict_train = np.zeros(train_arr.shape[0])
    for l in range(train_arr.shape[0]):
      y_predict_train[l] = y_predict(alpha_best,best_sigma[t],train_arr[:,0:12],train_arr[:,0:12])
    mse_ls_train[t] = mse(y_predict_train,train_arr[:,12])
    
    
    
  return mse_ls,best_gamma,best_sigma,mse_ls_train
